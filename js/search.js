...
function pausesall() 
{
	...
	for (var s = 0; s < stringra.length; s++)
	{
		...
		if (hier == "a") document.getElementById('stfb').innerHTML = '<table width=100%><tr><td width=1><img id="stfstop" src="images/ajax-loader.gif" title="click to stop search" width=25 onclick="stopsearch=1;"></td><td width=1>Search&nbsp;Results&nbsp;for&nbsp;<b>' + getstring + ':&nbsp;</b></td><td align=left> <a href="javascript:go_anchor(\'searchb\',\'sbfDN\')">DN:</a> <font id="stfd"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfMN\')">MN:</a> <font id="stfm"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfSN\')">SN:</a> <font id="stfs"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfAN\')">AN:</a> <font id="stfa"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfKN\')">KN:</a> <font id="stfk"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfVin\')">Vin:</a> <font id="stfv"></font></td><td width=1><input type="button" value="-" title="minimize search frame" onClick="moves(0); stopsearch=1"></td></tr></table>';
	else document.getElementById('stfb').innerHTML = '<table width=100%><tr><td width=1><img id="stfstop" src="images/ajax-loader.gif" title="click to stop search" width=25 onclick="stopsearch=1;"></td><td width=1>Search&nbsp;Results&nbsp;for&nbsp;<b>' + getstring + ':&nbsp;</b></td><td align=left> <a href="javascript:go_anchor(\'searchb\',\'sbfDN\')">DN:</a> <font id="stfd"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfMN\')">MN:</a> <font id="stfm"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfSN\')">SN:</a> <font id="stfs"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfAN\')">AN:</a> <font id="stfa"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfKN\')">KN:</a> <font id="stfk"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfVin\')">Vin:</a> <font id="stfv"></font>, <a href="javascript:go_anchor(\'searchb\',\'sbfAbhi\')">Abhi:</a> <font id="stfy"></font></td><td width=1><input type="button" value="-" title="minimize search frame" onClick="moves(0); stopsearch=1;"></td></tr></table>';
...
function importXMLs(cnt)
{
	...
	if (cnt == 1) // whole tipitaka
	{
			...
			{
				document.getElementById('sbfb').innerHTML += '<div id="sbf' + nikname[newnikaya] + '" title="xyz"><h1>' + nikname[newnikaya] + '</h1><hr></div>';
				thiscount = 0;
				rescount++;
			}
			...
			else if (window.ActiveXObject)
			{
				...
				xmlDoc.onreadystatechange = function () {
					if (xmlDoc.readyState == 4) createTables()
				};
			}
			else
			{
	...
	else if (cnt == 2) // nikaya
	{
		...
		if (qz == 0) {
			document.getElementById('sbfab').innerHTML = '';
			document.getElementById('sbfb').innerHTML = '<hr>';
			document.getElementById('stfb').innerHTML = '<table width=100%><tr><td width=1><img id="stfstop" src="images/ajax-loader.gif" title="click to stop search" width=25 onclick="stopsearch=1;"></td><td width=1>Search&nbsp;Results&nbsp;for&nbsp;<b>' + getstring + ':&nbsp;</b></td><td align=left><font id="stfx"></font> matches in ' + nikname[nikaya] + '</td><td width=1><input type="button" value="-" title="minimize search frame" onClick="stopsearch=1; moves(0)"></td></tr></table>';
		}
	...
	else if (cnt == 3) // this book
	{
		...
		document.getElementById('stfb').innerHTML = '<table width=100%><tr><td width=1><img id="stfstop" src="images/ajax-loader.gif" title="click to stop search" width=25 onclick="stopsearch=1;"></td><td align=left>Search Results for <b>' + getstring + '</b> in <b>' + nikname[nikaya] + ' ' + book + '</b>: <b id="num">' + countmatch + '</b></td><td width=1><input type="button" value="-" title="minimize search frame" onClick="stopsearch=1; moves(0)"></td></tr></table>';
...
function createTables()
{
						...
						for (var tmp = 0; tmp < z.length; tmp++) // per paragraph
						{
							...
							if (yesplus >= 0)					
							{
								...
								{								
									...
									exwordout = '<div><a class="white" href="javascript:void(0);" onclick="showonly(\'xyz\')">Show All</a></div><hr><table width=100%><tr>';
									...
									for(var t=0; t<exword.length; t++) {
										...
										for (ex = 0; ex < exnodups[t].length; ex++)
										{
											...
											exwordout += '<div><a class="white" href="javascript:void(0);" onclick="showonly(\'' + exnodups[t][ex].replace(/\"/g, 'x') + '\')">' + replaceunistandard(exnodups[t][ex]) + '</a> (' + dups[exnodups[t][ex]] + ')</div>';
										}
										exwordout += '</td>';
									}
									...
									finalout += ', Paragraph ' + (tmp + 1) + ' <input type="button" value="go" onclick="javascript:searchgo(\'' + bookfile + '\',' + (book - 1) + ',' + sx + ',' + sy + ',' + sz + ',' + s + ',' + se + ',' + tmp + ',\'' + sraout + '\',' + nummatch + ')"> <a href="#top">top</a></font><p>' + formatuniout(postpara,1) + '<hr></div>';
							...
							else
							{
								...
								if (startmatch >= 0)
								{
									...
									ctab = 0;
									exwordout = '<div><a class="white" href="javascript:void(0);" onclick="showonly(\'xyz\')">Show All</a></div><hr><table width=100%><tr><td valign="top">';
									for (ex = 0; ex < findiv; ex++)
									{
										
										exwordout += '<div><a class="white" href="javascript:void(0);" onclick="showonly(\'' + exnodups[ex].replace(/\"/g, 'x') + '\')">' + replaceunistandard(exnodups[ex]) + '</a> (' + dups[exnodups[ex]] + ')</div>';
										ctab++;
									}
									if(exnodups.length > 1)
									{
										...
										{
										exwordout += '<div><a class="white" href="#" onclick="showonly(\'' + exnodups[ex].replace(/\"/g, 'x') + '\')">' + exnodups[ex] + '</a> (' + dups[exnodups[ex]] + ')</div>';
										}	
									...
									if(x.length>1) finalout += '.' + (s+1);
									if(y.length>1) finalout += '.' + (se+1);
									finalout += ', Paragraph ' + (tmp + 1) + ' <input type="button" value="go" onclick="javascript:searchgo(\'' + bookfile + '\',' + (book - 1) + ',' + sx + ',' + sy + ',' + sz + ',' + s + ',' + se + ',' + tmp + ',\'' + sraout + '\',' + nummatch + ')"> <a class="white" href="#top">top</a></font><br><br>' + formatuniout(postpara,1) + '</p><hr></div>';
									nummatch += extranummatch; // add extra matches in this paragraph for next count.
									extranummatch = -1; 
									thiscount++;
									countmatch++;
	...
	if (match == 0) document.getElementById('sbfb').innerHTML += '<div title="xyz"><p><font size=4 class="green">' + nikname[nikaya] + ' ' + book + '</font> - <font class="yellow" size=3><i>No Match</i> <a class="white" href="#top">top</a></font><hr></div>';
...
{
	moves(0);
	if (stringra) document.getElementById('plus').innerHTML = '<input type="button" value="+" title="maximize search frame" onClick="moves(1)">';
	var ssect = se;
