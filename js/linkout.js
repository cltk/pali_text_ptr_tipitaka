...
function pedXMLout() {
	...
	if (tnum != 0) tout += '<input type="button" value="<" onclick="paliXML(\'PED/' + tloc1 + '/' + (tnum - 1) + '\')">';
	if (tnum != pedln[tloc2]) bout += '<input type="button" value=">" onclick="paliXML(\'PED/' + tloc1 + '/' + (tnum + 1) + '\')">';
...
function DPPNXML(file)
{
	...
	{
		xmlDoc = document.implementation.createDocument("", "", null);
		xmlDoc.onload = DPPNXMLout;
	}
	else if (window.ActiveXObject)
	{
		xmlDoc = new ActiveXObject("Microsoft.XMLDOM");
		xmlDoc.onreadystatechange = function () {
			if (xmlDoc.readyState == 4) DPPNXMLout()
		};
 	}
	else
	{
...
function DPPNXMLout() {
	...
	document.getElementById('difb').setAttribute('align','left');
	document.getElementById('difb').innerHTML = data;
	document.getElementById('leftbot').scrollTop = 0;
...
/*
	...
	if (tnum != 0) tout += '<input type="button" value="<" onclick="paliXML(\'etc/' + tloc1 + '/' + (tnum - 1) + '.xml\')">';
	if (tnum != pedln[tloc2]) bout += '<input type="button" value=">" onclick="paliXML(\'etc/' + tloc1 + '/' + (tnum + 1) + '.xml\')">';
*/
